...
hi def link hareFunc Function
hi def link hareTypedef Identifier
hi def hareType ctermfg=DarkCyan guifg=DarkCyan
"hi def hareThis ctermfg=DarkMagenta guifg=DarkMagenta
"syn match hareThis '\(\w\)\@<!this\(\w\)\@!'
syn match hareTypedef  contains=hareTypedef "\%([^[:cntrl:][:space:][:punct:][:digit:]]\|_\)\%([^[:cntrl:][:punct:][:space:]]\|_\)*" display contained
syn match hareFunc    "\%(r#\)\=\%([^[:cntrl:][:space:][:punct:][:digit:]]\|_\)\%([^[:cntrl:][:punct:][:space:]]\|_\)*" display contained
"syn keyword Keyword   def nextgroup=Function skipwhite skipempty
syn keyword hareKeyword struct enum namespace type nextgroup=hareTypedef skipwhite skipempty
syn keyword hareKeyword union nextgroup=hareType skipwhite skipempty contained
syn match hareFunc    "\w\(\w\)*("he=e-1,me=e-1
syn match hareFunc    "\w\(\w\)*<"he=e-1,me=e-1 " foo<T>();
syn match hareType    "\w\(\w\)*::[^<]"he=e-3,me=e-3
